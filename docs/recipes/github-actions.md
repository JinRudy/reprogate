...
## Comment On Missing Evidence

Use this when you want ReproGate to leave one maintainer-style comment on issues
or pull requests that are missing reproduction details. The Action updates its
previous comment instead of adding a new one on every edit.
...
ReproGate only comments on issue or pull request events, only when required
evidence is missing, and reuses the same hidden comment marker so repeated edits
do not create a comment thread.

If you also want labels, route the `labels` output into your existing labeler or
triage workflow. See the routing example above.
